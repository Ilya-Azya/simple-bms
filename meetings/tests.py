...
def test_meeting_str():
    ...
    m = Meeting.objects.create(title="Daily", team=team, start_at=start, end_at=end, created_by=user)
    s = str(m)
    assert "Daily" in s
...
def test_meeting_overlap_same_team_invalid():
    ...
    Meeting.objects.create(title="A", team=team, start_at=s1, end_at=e1, created_by=user)
    with pytest.raises(ValidationError) as e:
...
def test_meeting_overlap_different_team_ok():
    ...
    Meeting.objects.create(title="A", team=team1, start_at=s1, end_at=e1, created_by=user)
    Meeting.objects.create(title="B", team=team2, start_at=s1, end_at=e1, created_by=user)
...
def test_participant_overlap_invalid_via_model_clean():
    ...
    m1 = Meeting.objects.create(title="A", team=team, start_at=s1, end_at=e1, created_by=u)
...
def test_meeting_end_equal_start_invalid():
    ...
    with pytest.raises(ValidationError) as e:
        Meeting.objects.create(title="bad", team=team, start_at=start, end_at=end, created_by=u)
    assert "End of meeting should be later than start" in str(e.value)
...
def test_form_participants_queryset_from_instance_on_edit():
    ...
    s, e = make_times()
    meeting = Meeting.objects.create(title="X", team=team, start_at=s, end_at=e, created_by=u)
...
def test_form_rejects_non_team_participant():
    ...
    u_out = User.objects.create_user(username="out", email="out@test.com", password="pass")
...
def test_list_view_user_perms_flags(client):
    t1 = Team.objects.create(name="T1")
    t2 = Team.objects.create(name="T2")
    user = User.objects.create_user(username="u", email="u@test.com", password="pass", role="Team Admin")
    s1, e1 = make_times(1, 1)
    s2, e2 = make_times(2, 1)
    m1 = Meeting.objects.create(title="A", team=t1, start_at=s1, end_at=e1, created_by=user)
    m2 = Meeting.objects.create(title="B", team=t2, start_at=s2, end_at=e2, created_by=user)
...
def test_detail_view_context_flags(client):
    team = Team.objects.create(name="T")
    manager = User.objects.create_user(username="mgr", email="mgr@test.com", password="pass")
    member = User.objects.create_user(username="mem", email="mem@test.com", password="pass")
    ...
    s, e = make_times()
    meeting = Meeting.objects.create(title="A", team=team, start_at=s, end_at=e, created_by=manager)
...
def test_create_view_allowed_for_admin_or_manager(client, role):
    ...
    if role == "MANAGER":
        TeamMembership.objects.create(team=team, user=user, role=TeamMembership.Role.MANAGER)
...
def test_update_view_forbidden_for_member(client):
    ...
    TeamMembership.objects.create(team=team, user=member, role=TeamMembership.Role.MEMBER)
    s, e = make_times()
    m = Meeting.objects.create(title="A", team=team, start_at=s, end_at=e, created_by=member)
...
def test_update_view_ok_for_team_admin(client):
    ...
    s, e = make_times()
    m = Meeting.objects.create(title="A", team=team, start_at=s, end_at=e, created_by=admin)
...
def test_delete_view_forbidden_for_member(client):
    ...
    TeamMembership.objects.create(team=team, user=member, role=TeamMembership.Role.MEMBER)
    s, e = make_times()
    m = Meeting.objects.create(title="A", team=team, start_at=s, end_at=e, created_by=user)
...
def test_delete_view_ok_for_team_admin(client):
    ...
    resp_post = client.post(reverse("meetings:meeting_delete", args=[m.pk]), follow=True)
...
def test_list_pagination_page2(client):
    team = Team.objects.create(name="T")
    admin = User.objects.create_user(username="admin", email="admin@test.com", password="pass", role="Team Admin")

    for i in range(25):
        s, e = make_times(offset_hours=i + 1)
        Meeting.objects.create(title=f"M{i}", team=team, start_at=s, end_at=e, created_by=admin)
    client.login(username="admin", password="pass")
